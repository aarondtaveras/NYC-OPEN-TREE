...
{
	double result = haversine( lat1, lon1, latitude, longitude);
...
{
	string t1_copy = t1.spc_common;
	string t2_copy = t2.spc_common;
	for(auto & c: t1_copy){
		c = tolower(c);
	}
	for(auto & d: t2_copy){
	...
	return t1_copy.size() == t2_copy.size();
...
bool operator==(const Tree & t1, const Tree & t2)
{
	if(t1.spc_common.size()==t2.spc_common.size() && t1.tree_id == t2.tree_id){
		return true;
	}
	return false;
}
